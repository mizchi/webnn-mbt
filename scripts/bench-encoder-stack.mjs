...
const port = Number(process.env.BENCH_PORT ?? 4186);
...
await withCanaryPage({ port }, async ({ page, browserVersion }) => {
  await page.waitForFunction(
    () =>
      typeof globalThis.webnnPlayground?.benchmarkTransformerEncoderStack ===
      "function",
  );
  const reports = [];
    ...
    reports.push(
      await page.evaluate(
        ([
          ...
          globalThis.webnnPlayground.benchmarkTransformerEncoderStack(
            count,
            batch,
            tokenCount,
            modelWidth,
  ...
  console.log(
    `\nChrome ${browserVersion} Canary headless / Transformer encoder stack / ` +
  ...
  for (const report of reports) {
    ...
    console.log(
      `| ${report.layers} | ${report.hiddenSize} | ` +
